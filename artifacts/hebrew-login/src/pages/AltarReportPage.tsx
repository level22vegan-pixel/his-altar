...
type RosterCampus = { campus: string; source: "checkins" | "roster"; members: { name: string; role: string; category: string }[] };

async function exportActivesPerService(dateStr: string, service: string) {
  const res = await fetch(`/api/check-ins/roster?serviceDate=${encodeURIComponent(dateStr)}&service=${encodeURIComponent(service)}`);
  const data = await res.json() as { campuses: RosterCampus[] };
  ...
  doc.setFontSize(10);
  doc.setTextColor(...PDF_GOLD_DIM);
  doc.text(`${dateLabel} — ${service}`, W / 2, 24, { align: "center" });
  doc.setDrawColor(...PDF_GOLD_DIM);
  doc.setLineWidth(0.3);
  doc.line(14, 27, W - 14, 27);
  ...
  const startX = 14;
  const rowH = 8;
  const colW = [55, 55, 30, 30] as const;
  const cols = ["Name", "Role", "Category", "Source"];

  let grandTotal = 0;

  for (const { campus, source, members } of data.campuses) {
    ...
    doc.setFontSize(10);
    doc.setTextColor(...PDF_GOLD);
    doc.text(campus, startX + 3, y);
    const sourceLabel = source === "checkins" ? "checked in" : "roster";
    doc.setFont("times", "italic");
    doc.setFontSize(8);
    doc.setTextColor(...PDF_GOLD_DIM);
    doc.text(`${members.length} members (${sourceLabel})`, W - 14, y, { align: "right" });
    y += rowH;
    ...
    members.forEach((m, ri) => {
      ...
      doc.setTextColor(...PDF_WHITE);
      x = startX + 2;
      [m.name, m.role, m.category, source === "checkins" ? "✓ Check-in" : "Roster"].forEach((cell, i) => {
  ...
  doc.setFontSize(9);
  doc.setTextColor(...PDF_GOLD);
  doc.text(`TOTAL ACTIVE MEMBERS: ${grandTotal}`, startX + 2, y);
  ...
  doc.save(`actives-${dateStr}-${service.replace(/\s+/g, "-")}.pdf`);
}
    ...
    <div style={{ marginBottom: 8 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, paddingBottom: 8, borderBottom: `1px solid ${BORDER}` }}>
        ...
        <div style={{ display: "flex", gap: 5, marginLeft: "auto", flexShrink: 0 }}>
          ...
          <button
            onClick={() => exportActivesPerService(dateStr, service)}
            title="Export active altar members for this service"
            style={{ color: GOLD_DIM, background: "hsl(38 30% 14%)", border: `1px solid hsl(38 25% 22%)`, borderRadius: 5, padding: "4px 8px", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", transition: "all 0.15s", whiteSpace: "nowrap" }}
            onMouseOver={e => { e.currentTarget.style.color = GOLD; }}
            onMouseOut={e => { e.currentTarget.style.color = GOLD_DIM; }}
          >
