    ...
    >
      ...
      <div className="relative z-10 w-full max-w-xl px-4 pt-14 pb-20">
        <div className="text-center mb-8">
          <h1 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(2rem, 6vw, 3rem)", color: "hsl(0 0% 97%)", letterSpacing: "0.28em", textTransform: "uppercase" }}>
            PXP
          </h1>
        ...
        <div style={{ background: "hsl(220 12% 14%)", border: "1px solid hsl(220 12% 22%)", borderRadius: 12, padding: 20, marginBottom: 16 }}>
          <p style={{ color: "hsl(220 15% 50%)", fontFamily: "Georgia, serif", fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 14 }}>
            Step 1 — Caller Info
          </p>
            ...
            {isCallerSession ? (
              <>
                <div style={{ ...inputStyle, display: "flex", alignItems: "center", justifyContent: "space-between", opacity: 0.75 }}>
                  <span style={{ color: "hsl(210 55% 70%)" }}>{lockedCampus}</span>
                  <span style={{ color: "hsl(220 10% 45%)", fontSize: 10, letterSpacing: "0.1em" }}>CAMPUS</span>
                </div>
                <div style={{ ...inputStyle, display: "flex", alignItems: "center", justifyContent: "space-between", opacity: 0.75 }}>
                  <span style={{ color: "hsl(210 55% 70%)" }}>{lockedCallerName}</span>
                  <span style={{ color: "hsl(220 10% 45%)", fontSize: 10, letterSpacing: "0.1em" }}>YOUR NAME</span>
                </div>
              </>
            ) : (
              <>
                ...
                {lockedCampus && (
                  <div style={{ ...inputStyle, display: "flex", alignItems: "center", justifyContent: "space-between", opacity: 0.75 }}>
                    <span style={{ color: "hsl(210 55% 70%)" }}>{lockedCampus}</span>
                    <span style={{ color: "hsl(220 10% 45%)", fontSize: 10, letterSpacing: "0.1em" }}>CAMPUS LOCKED</span>
                  </div>
                )}
                {callers.length > 0 ? (
                  <>
        ...
        <div style={{ background: "hsl(220 12% 14%)", border: "1px solid hsl(220 12% 22%)", borderRadius: 12, padding: 20, marginBottom: 20 }}>
          <p style={{ color: "hsl(220 15% 50%)", fontFamily: "Georgia, serif", fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 14 }}>
            Step 2 — Select Contact to Call
            {lockedCampus && <span style={{ marginLeft: 8, opacity: 0.6 }}>({lockedCampus})</span>}
          </p>

          <div style={{ display: "flex", background: "hsl(220 15% 8%)", borderRadius: 8, padding: 3, marginBottom: 12, gap: 3 }}>
            {(["all", "called"] as const).map(tab => (
              <button
                ...
                style={{
                  ...
                  padding: "7px 0",
                  borderRadius: 6,
                  background: contactFilter === tab ? "hsl(210 40% 18%)" : "transparent",
                  color: contactFilter === tab ? "hsl(210 65% 75%)" : "hsl(220 10% 42%)",
                  fontFamily: "Georgia, serif",
                  fontSize: 11,
                  letterSpacing: "0.15em",
                  textTransform: "uppercase",
                  border: contactFilter === tab ? "1px solid hsl(210 45% 32%)" : "1px solid transparent",
                  cursor: "pointer",
          ...
          <div style={{ marginBottom: 10 }}>
            <select
              style={{ ...inputStyle, appearance: "none" as const, color: serviceFilter ? "hsl(210 60% 72%)" : "hsl(220 10% 50%)" }}
              value={serviceFilter}
          ...
          <div style={{ display: "flex", background: "hsl(220 15% 8%)", borderRadius: 8, padding: 3, marginBottom: 10, gap: 3 }}>
            {(["newest", "oldest"] as const).map(opt => (
              <button
                ...
                style={{
                  ...
                  background: sortOrder === opt ? "hsl(210 40% 18%)" : "transparent",
                  color: sortOrder === opt ? "hsl(210 65% 75%)" : "hsl(220 10% 42%)",
                  fontFamily: "Georgia, serif", fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase",
                  border: sortOrder === opt ? "1px solid hsl(210 45% 32%)" : "1px solid transparent",
          ...
          <div style={{ maxHeight: 280, overflowY: "auto", borderRadius: 8, border: "1px solid hsl(220 12% 18%)" }}>
            {contacts.length === 0 ? (
              <div style={{ padding: 24, textAlign: "center", color: "hsl(220 10% 40%)", fontFamily: "Georgia, serif", fontSize: 12 }}>
                {search
                  ? "No contacts match"
                ...
                return (
                  <div
                    ...
                    style={{
                      display: "flex",
                      alignItems: "center",
                      borderBottom: i < contacts.length - 1 ? "1px solid hsl(220 12% 15%)" : "none",
                      background: isSelected ? "hsl(210 40% 16%)" : i % 2 === 0 ? "transparent" : "hsl(220 12% 11%)",
                      outline: isSelected ? "2px solid hsl(210 55% 42%)" : "none",
                      outlineOffset: -2,
                    }}
                  >
                    <button
                    ...
                    >
                      <div style={{
                        ...
                        background: isSelected ? "hsl(210 50% 30%)" : `hsl(${(c.id * 47) % 360} 40% 25%)`,
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 4 }}>
                          <span style={{ color: isSelected ? "hsl(210 70% 82%)" : "hsl(0 0% 90%)", fontFamily: "Georgia, serif", fontSize: 13, fontWeight: "bold" }}>
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...
                                  border: `2px solid ${color ?? "hsl(220 15% 26%)"}`,
                                  display: "flex",
                                  alignItems: "center",
                              ...
                              >
                                <span style={{ color: color ? "hsl(0 0% 97%)" : "hsl(220 10% 36%)", fontSize: idx === 0 ? 12 : idx === 1 ? 11 : 10, letterSpacing: "0.04em" }}>
                                  {"📞".repeat(idx + 1)}
                                </span>
                                {attempt && (
                        ...
                        <div style={{ color: "hsl(220 10% 48%)", fontFamily: "Georgia, serif", fontSize: 11, marginTop: 4 }}>
                          {formatPhone(c.phone)}{!lockedCampus && c.campus ? ` · ${c.campus}` : ""}
                        </div>
                      </div>
                      {isSelected && (
                        <span style={{ color: "hsl(210 65% 65%)", fontSize: 15 }}>✓</span>
                      )}
                    </button>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: "none",
                        border: "none",
                        borderLeft: "1px solid hsl(220 12% 16%)",
                        color: "hsl(220 10% 42%)",
                        cursor: "pointer",
                        fontFamily: "Georgia, serif",
                        fontSize: 13,
                      ...
                      onMouseOver={e => { e.currentTarget.style.color = "hsl(210 55% 65%)"; }}
                      onMouseOut={e => { e.currentTarget.style.color = "hsl(220 10% 42%)"; }}
                    >
                      ›
                    </button>
                  </div>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 12,
            background: canStart
              ? "linear-gradient(135deg, hsl(210 55% 38%), hsl(210 50% 28%))"
              : "hsl(220 12% 16%)",
            color: canStart ? "hsl(0 0% 97%)" : "hsl(220 10% 35%)",
            fontFamily: "Georgia, serif",
            fontSize: 14,
            letterSpacing: "0.25em",
            textTransform: "uppercase",
            border: `1px solid ${canStart ? "hsl(210 50% 42%)" : "hsl(220 12% 22%)"}`,
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
          <button
            ...
            style={{ flex: 1, padding: "11px 0", borderRadius: 10, background: "hsl(220 12% 14%)", color: "hsl(220 10% 58%)", fontFamily: "Georgia, serif", fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", border: "1px solid hsl(220 12% 22%)", cursor: "pointer" }}
          >
            Call History
          </button>
          <button
