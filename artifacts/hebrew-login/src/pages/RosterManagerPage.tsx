...
function ServiceLeadSection({ campus }: { campus: string }) {
  const [code, setCode] = useState("");
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [err, setErr] = useState("");
  ...
  useEffect(() => {
    fetch("/api/campus-passwords")
      ...
      .then(d => {
        const has = (d.passwords ?? []).some(
          (p: { campus: string; role: string }) => p.campus === campus && p.role === "attendance"
        );
        setExisting(has);
      })
      .catch(() => {});
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch("/api/campus-passwords", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ campus, role: "attendance", password: code }),
      });
      if (!res.ok) throw new Error();
      setSaved(true);
      setCode("");
  ...
  return (
    <div style={{ marginTop: 32, borderTop: "1px solid hsl(38 15% 18%)", paddingTop: 24 }}>
      <p style={{ color: "hsl(38 35% 50%)", fontFamily: "Georgia, serif", fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 4 }}>
        Service Lead Access
      </p>
      <p style={{ color: "hsl(38 22% 36%)", fontFamily: "Georgia, serif", fontSize: 12, letterSpacing: "0.04em", marginBottom: 14 }}>
        Service leads use this 4-digit code at the login screen to access attendance only — no team page, no admin tools.
        {existing && <span style={{ color: "hsl(130 45% 48%)", marginLeft: 8 }}>✓ Code is set for {campus}</span>}
      </p>
      <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
        <div style={{ flex: 1 }}>
          <label style={LABEL}>{existing ? "Update" : "Set"} Attendance Code</label>
          <input
            value={code}
            onChange={e => { setCode(e.target.value.replace(/\D/g, "").slice(0, 4)); setErr(""); setSaved(false); }}
            placeholder="4-digit code"
      ...
      {err && <p style={{ color: "hsl(0 55% 55%)", fontFamily: "Georgia, serif", fontSize: 12, marginTop: 8 }}>{err}</p>}
      {saved && <p style={{ color: "hsl(130 45% 52%)", fontFamily: "Georgia, serif", fontSize: 12, marginTop: 8 }}>Attendance code updated for {campus}.</p>}
    </div>
  );
}
