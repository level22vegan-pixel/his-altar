...
const PLAN_COLORS: Record<string, string> = {
  free: "#475569", starter: "#1d4ed8", pro: "#7c3aed", enterprise: "#b45309",
};
const STATUS_COLORS: Record<string, string> = {
  active: "#16a34a", trial: "#0891b2", past_due: "#d97706", cancelled: "#dc2626", suspended: "#7f1d1d",
};
...
function Badge({ label, color }: { label: string; color: string }) {
  return (
    <span style={{
      ...
      color: color, borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 600,
...
export default function SuperAdminDashboard() {
  ...
  async function expand(org: OrgRow) {
    ...
    setMsgs(mData.messages ?? []);
  }

  async function saveBilling(orgId: number) {
    setSaving(true);
    try {
      await saFetch(`/orgs/${orgId}`, { method: "PUT", body: JSON.stringify(editOrg) });
      await load();
  ...
  async function sendMsg(orgId: number) {
    ...
    try {
      ...
      setNewMsg("");
      const mRes = await saFetch(`/orgs/${orgId}/messages`);
  ...
  const inp: React.CSSProperties = {
    ...
    color: "#f1f5f9", fontSize: 13, padding: "8px 12px", outline: "none", width: "100%", boxSizing: "border-box",
  };
    ...
    <div style={{ minHeight: "100vh", background: "#080c14", color: "#f1f5f9", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{ background: "#0f172a", borderBottom: "1px solid #1e293b", padding: "0 28px", display: "flex", alignItems: "center", height: 58 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 15, color: "#f1f5f9" }}>Site Control</span>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 28px" }}>
        ...
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 16, overflow: "hidden" }}>
          ...
          <div style={{ padding: "20px 24px", borderBottom: "1px solid #1e293b", display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ position: "relative" }}>
              <input
                ...
                style={{ ...inp, width: 220, paddingLeft: 34 }}
              />
            ...
            <button onClick={load} style={{ ...btn("#1e293b"), border: "1px solid #334155", color: "#94a3b8" }}>↻ Refresh</button>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: 48, color: "#475569" }}>
              {search ? "No churches match your search." : "No organizations yet."}
            </div>
          ) : (
            <>
              {/* Column headers */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 180px 100px 110px 90px 90px 120px", gap: 0, padding: "10px 24px", borderBottom: "1px solid #1e293b" }}>
              ...
              {filtered.map(org => (
                <div key={org.id}>
                  ...
                  <div
                    onClick={() => expand(org)}
                    style={{
                      display: "grid", gridTemplateColumns: "1fr 180px 100px 110px 90px 90px 120px",
                      gap: 0, padding: "14px 24px", borderBottom: "1px solid #1e293b",
                      cursor: "pointer", transition: "background 0.1s",
                      background: expanded === org.id ? "#0d1626" : "transparent",
                    }}
                  ...
                  >
                    ...
                    <div style={{ alignSelf: "center", textAlign: "right" }}>
                      <span style={{ color: "#475569", fontSize: 13 }}>{expanded === org.id ? "▲" : "▼"}</span>
                    </div>
                  </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                            <div>
                              ...
                              <select value={editOrg.plan ?? org.plan} onChange={e => setEditOrg(p => ({ ...p, plan: e.target.value }))}
                            ...
                            <div>
                              ...
                              <select value={editOrg.billingStatus ?? org.billingStatus} onChange={e => setEditOrg(p => ({ ...p, billingStatus: e.target.value }))}
                                style={{ ...inp, cursor: "pointer" }}>
                            ...
                            <div>
                              ...
                              <textarea value={editOrg.billingNotes ?? ""} onChange={e => setEditOrg(p => ({ ...p, billingNotes: e.target.value }))}
                                placeholder="Payment info, notes, Stripe customer ID…"
                                rows={3}
                                style={{ ...inp, resize: "vertical", fontFamily: "inherit" }} />
                            </div>
                            <button onClick={() => saveBilling(org.id)} disabled={saving} style={btn()}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                            ...
                            <div style={{ display: "flex", gap: 8 }}>
                              <button
                                ...
                                style={btn(org.suspended ? "#16a34a" : "#b45309")}
                              >
                            ...
                            {confirmDelete === org.id && (
                              <div style={{ background: "#1c0a0a", border: "1px solid #7f1d1d", borderRadius: 10, padding: 14 }}>
                                <p style={{ color: "#fca5a5", fontSize: 13, margin: "0 0 12px" }}>
                                  Permanently delete <strong>{org.name}</strong>? This cannot be undone.
                                </p>
                                <div style={{ display: "flex", gap: 8 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                          <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 10, padding: 16, minHeight: 160, maxHeight: 300, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
                            {msgs.length === 0 ? (
                              ...
                              <div key={m.id} style={{ display: "flex", flexDirection: m.fromAdmin ? "row-reverse" : "row", gap: 8 }}>
                                <div style={{
                                  maxWidth: "75%", background: m.fromAdmin ? "#1d3a6e" : "#1e293b",
                                  border: `1px solid ${m.fromAdmin ? "#2563eb44" : "#33415544"}`,
                          ...
                          <div style={{ display: "flex", gap: 10 }}>
                            <input
                              ...
                              onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMsg(org.id); } }}
                              placeholder="Send a message to this church…"
                              style={{ ...inp, flex: 1 }}
                            />
                            <button onClick={() => sendMsg(org.id)} disabled={sending || !newMsg.trim()} style={btn()}>
                              {sending ? "…" : "Send"}
                            </button>
                          </div>
                        </div>
        ...
        <p style={{ textAlign: "center", color: "#1e293b", fontSize: 12, marginTop: 32 }}>
          His Altar · Site Control · Platform Administrator
        </p>
      </div>
    </div>
  );
