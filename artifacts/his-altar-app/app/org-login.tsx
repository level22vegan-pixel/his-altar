...
export default function OrgLoginScreen() {
  ...
  async function handleLogin() {
    ...
    try {
      await loginOrg(email.trim(), password);
      router.replace("/team");
    } catch (e: any) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={[styles.root, { paddingTop: topPad + 20 }]} keyboardShouldPersistTaps="handled">
        ...
        <Text style={[styles.title, { color: colors.foreground }]}>CHURCH SIGN IN</Text>
        <Text style={[styles.sub, { color: colors.mutedForeground }]}>Access your ministry portal</Text>
        <View style={styles.form}>
          ...
          <GoldButton label="Sign In" onPress={handleLogin} loading={loading} />
          <Pressable onPress={() => router.push("/org-signup")} style={styles.link}>
            <Text style={[styles.linkText, { color: colors.primary }]}>New church? Register here</Text>
          </Pressable>
        </View>
      </ScrollView>
